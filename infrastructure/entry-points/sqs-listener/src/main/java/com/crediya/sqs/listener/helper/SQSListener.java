...
public class SQSListener {
    ...
    private Flux<Void> listenRetryRepeat() {
        return listen()
                .doOnError(e -> log.error("Error escuchando cola SQS", e))
                .repeat();
    }
    ...
    private Flux<Void> listen() {
        return getMessages()
                ...
                .onErrorContinue((e, o) -> log.error("Error escuchando mensaje SQS", e));
    }
    ...
    private Flux<Message> getMessages() {
        return Mono.fromCallable(this::getReceiveMessageRequest)
                ...
                .doOnNext(response -> log.debug("{} mensajes recibidos de SQS", response.messages().size()))
